...
**Every review answers one defined question and has a closure condition.** Record which question this review resolves (for example "is the gate logic in `eval/gates.py` sound before the AP-1 run?") and what counts as closed. Default to one scoped review, then one verification of its repairs — the verification covers the changed behavior and the named findings, not another general audit; a further substantive review consumes an explicitly allocated additional allowance. Once a finding is accepted as fixed, it reopens only on new relevant evidence or changed semantics; a document's new filename or a cosmetic edit never reopens it.
...
2. **Group findings by failure mechanism**, not by count. Five comments about the same missing null check are one finding. A finding is blocking only when it names the required behavior, concrete evidence, and the practical consequence — substantiate it with a reproducer, a demonstrated call path, a contradicting result, or a binding contract citation; important uncertainty can still prevent acceptance, but report it as uncertainty, not as a proved bug. After a demonstrated failure, check the same mechanism across a bounded related surface once; do not widen to an unscoped repository audit without evidence that the scope is that wide.
...
5. **Present three buckets to the user:** actionable (with the evidence), rejected (with the one-line reason — never silently drop a finding), unresolved (what would settle it). Optional or cosmetic suggestions go into a short backlog note rather than a fourth bucket, and never trigger automatic follow-up work on their own. Lead with the verdict and the 3–7 most consequential items; give the output path for the rest.
