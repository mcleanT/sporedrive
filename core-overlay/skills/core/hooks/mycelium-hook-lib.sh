...
# --- request-reduction v1 (items 3 and 4): runtime accounting helpers ----------
# Housekeeping attempt accounting. Prints the ledger helper's JSON decision for
# KIND. Any helper failure (missing helper, busy ledger, bad output) prints a
# skip decision so a broken ledger can never dispatch a worker. No model, no
# dispatch here: this only accounts. Overrides: MYCELIUM_HOUSEKEEPING_LEDGER_HELPER,
# MYCELIUM_HOUSEKEEPING_MAX_ATTEMPTS (3), MYCELIUM_HOUSEKEEPING_COOLDOWN_MINUTES (360),
# MYCELIUM_HOUSEKEEPING_INFLIGHT_TTL_MINUTES (120), MYCELIUM_HOUSEKEEPING_INTERVAL_HOURS (24).
mycelium_housekeeping_decide() {
  local kind="$1"
  local ledger="$2"
  local last_success_ts="${3:-0}"
  local dispatcher="${4:-}"
  local here
  here="${HERE:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
  local helper="${MYCELIUM_HOUSEKEEPING_LEDGER_HELPER:-$here/../scripts/housekeeping_ledger.py}"
  local out=""
  [[ "$last_success_ts" =~ ^[0-9]{1,18}$ ]] || last_success_ts=0
  if [[ ! -f "$helper" ]]; then
    printf '{"action": "skip", "reason": "helper_missing", "notify": false, "kind": "%s"}\n' "$kind"
    return 0
  fi
  if out=$(python3 "$helper" decide "$kind" --ledger "$ledger" \
      --last-success-ts "$last_success_ts" --dispatcher "$dispatcher" \
      --interval-hours "${MYCELIUM_HOUSEKEEPING_INTERVAL_HOURS:-24}" \
      --max-attempts "${MYCELIUM_HOUSEKEEPING_MAX_ATTEMPTS:-3}" \
      --cooldown-minutes "${MYCELIUM_HOUSEKEEPING_COOLDOWN_MINUTES:-360}" \
      --inflight-ttl-minutes "${MYCELIUM_HOUSEKEEPING_INFLIGHT_TTL_MINUTES:-120}" 2>/dev/null) \
    && [[ "$out" == \{* ]]; then
    printf '%s\n' "$out"
  else
    printf '{"action": "skip", "reason": "helper_failed", "notify": false, "kind": "%s"}\n' "$kind"
  fi
  return 0
}

# Stop-lock contention accounting. Returns 0 when this is the FIRST unresolved
# contention for (this session, this live lock owner) -- the caller may emit
# its single actionable notification -- and 1 on a repeat of the same unresolved
# condition, which the caller must NOT turn into another model retry prompt.
# The sentinel is durable evidence (owner pid/ts, first/last seen, repeats) and
# is cleared by mycelium_stop_lock_contention_clear once the lock is acquired.
# Live owners are never force-unlocked here; dead owners are already reclaimed
# inside mycelium_acquire_stop_lock.
mycelium_stop_lock_contention_first() {
  local state_dir="$1"
  local session_id="${2:-}"
  local sentinel="$state_dir/mycelium-stop-lock-contention.json"
  local owner_pid="" owner_ts="" now_ts=""
  local lock_dir="$state_dir/mycelium-stop.lock"
  if [[ -f "$lock_dir/owner" ]]; then
    read -r owner_pid owner_ts < "$lock_dir/owner" || true
  fi
  [[ "$owner_pid" =~ ^[0-9]+$ ]] || owner_pid=0
  [[ "$owner_ts" =~ ^[0-9]+$ ]] || owner_ts=0
  now_ts=$(date +%s)
  MYCELIUM_STOP_LOCK_BUSY_OWNER="$owner_pid"
  python3 - "$sentinel" "$session_id" "$owner_pid" "$owner_ts" "$now_ts" <<'PY'
import json, os, sys
sentinel, session_id, owner_pid, owner_ts, now = sys.argv[1:6]
owner_pid, owner_ts, now = int(owner_pid), int(owner_ts), int(now)
prev = None
try:
    ...
    prev = None
first = True
rec = {"session_id": session_id, "owner_pid": owner_pid, "owner_ts": owner_ts,
       "first_seen_at": now, "last_seen_at": now, "repeats": 0, "notified": True,
       "state": "unresolved"}
if isinstance(prev, dict) and prev.get("session_id") == session_id \
        and int(prev.get("owner_pid") or 0) == owner_pid and prev.get("state") == "unresolved":
    first = False
    rec = dict(prev)
    rec["last_seen_at"] = now
    rec["repeats"] = int(rec.get("repeats") or 0) + 1
tmp = "%s.tmp-%d" % (sentinel, os.getpid())
...
mycelium_stop_lock_contention_clear() {
  local state_dir="$1"
  local sentinel="$state_dir/mycelium-stop-lock-contention.json"
  [[ -f "$sentinel" ]] || return 0
  python3 - "$sentinel" "$(date +%s)" <<'PY'
import json, os, sys
sentinel, now = sys.argv[1], int(sys.argv[2])
try:
    with open(sentinel) as fh:
        rec = json.load(fh)
except Exception:
    rec = {}
if isinstance(rec, dict) and rec.get("state") == "unresolved":
    # Keep the evidence, mark it reconciled (never delete lineage/evidence).
