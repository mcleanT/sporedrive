...
if ! mycelium_acquire_stop_lock "$STATE_DIR"; then
  ...
  # would bypass lifecycle enforcement. Preserve state.
  # request-reduction v1 (item 4): bounded contention is handled here, in runtime
  # code. The bounded in-hook wait already ran (dead owners reclaimed, live owners
  # never force-unlocked). The FIRST unresolved contention for this session and
  # live owner emits exactly ONE actionable block with the evidence path; a repeat
  # Stop under the same unresolved condition emits nothing (no further model
  # retry prompts) -- active state stays preserved for the next SessionStart /
  # Stop reconciliation, which clears the sentinel once the lock is acquired.
  if mycelium_stop_lock_contention_first "$STATE_DIR" "$HOST_SESSION_ID"; then
    mycelium_emit_stop_block \
      "STOP BLOCKED — the lifecycle transaction lock remained busy (live owner pid ${MYCELIUM_STOP_LOCK_BUSY_OWNER:-unknown}). Active state was preserved. This is the only retry prompt for this condition: retry Stop once; if it is still busy the host will stop and the next SessionStart reconciles. Evidence: ${STATE_DIR}/mycelium-stop-lock-contention.json"
  fi
  exit 0
fi
