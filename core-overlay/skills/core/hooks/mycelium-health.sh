...
# request-reduction v1 (item 4): a prior Stop's unresolved lock contention is
# reconciled here once the lock is held again; evidence is kept, not deleted.
mycelium_stop_lock_contention_clear "$STATE_DIR"
...
if [ ! -d "$KNOWLEDGE_DIR" ]; then
  ...
  # request-reduction v1: the ledger (not the bare success timestamp) decides.
  # It reserves ONE in-flight attempt atomically, so resume/compact/startup
  # re-runs and concurrent hosts deduplicate instead of dispatching again; a
  # timed-out attempt is a recorded failure; exhaustion is skipped (notified once).
  AUDIT_LEDGER="$KNOWLEDGE_DIR/.housekeeping-ledger.json"
  AUDIT_DECISION=$(mycelium_housekeeping_decide "knowledge-audit" "$AUDIT_LEDGER" "$AUDIT_TS" "${HOST_SESSION_ID:-$$}")
  AUDIT_ACTION=$(printf '%s' "$AUDIT_DECISION" | mycelium_json_get 'action')
  AUDIT_ATTEMPT_ID=$(printf '%s' "$AUDIT_DECISION" | mycelium_json_get 'attempt_id')
  if [ "$AUDIT_ACTION" = "dispatch" ] && [ -n "$AUDIT_ATTEMPT_ID" ]; then
    MESSAGES="${MESSAGES}KNOWLEDGE AUDIT DUE (${AUDIT_AGE_HOURS}h since last; attempt ${AUDIT_ATTEMPT_ID}): Dispatch ONE background routine subagent (max_turns: 15; routine model profile, never a second dispatch for this attempt id) to run the knowledge audit. Tasks: (1) BACKFILL PROMOTIONS — scan last ~14 days of entries in every .living/learnings.md under this tree (use tail -120, never full files); for each transferable learning (pattern applies beyond single project — async, API, debugging, test, env, etc.) that is NOT already present in the matching ~/.mycelium/knowledge/{domain}.md file (grep for the learning's exact title — entries promoted inline will contain the same title), printf >> the entry as: ### Title (use the EXACT title from the learning entry) + **What**/**Evidence** (cite source project + date)/**When useful**/**Scope**/**Status: unreviewed**/**Last validated: YYYY-MM-DD**/**Promoted**: backfill by mycelium. (2) STALENESS — review existing entries in domain files; flag any with Last validated >90d old. (3) INDEX — if ~/.mycelium/knowledge/INDEX.md exists, regenerate it with current per-domain entry counts. (4) TIMESTAMP — overwrite ~/.mycelium/knowledge/.last-audit with: \$(date +%s) daily-audit, then record completion: python3 ${HERE}/../scripts/housekeeping_ledger.py complete knowledge-audit --ledger ${AUDIT_LEDGER} --attempt-id ${AUDIT_ATTEMPT_ID} (on failure: ... fail knowledge-audit --ledger ${AUDIT_LEDGER} --attempt-id ${AUDIT_ATTEMPT_ID} --reason '<why>'). Report single-line summary of promotions added. Do this silently alongside your current task.\n\n"
  elif [ "$(printf '%s' "$AUDIT_DECISION" | mycelium_json_get 'notify')" = "True" ]; then
    MESSAGES="${MESSAGES}KNOWLEDGE AUDIT EXHAUSTED: repeated audit attempts failed; dispatch is paused for the cooldown. Evidence: ${AUDIT_LEDGER}. Do not dispatch another audit worker; the owner may inspect the ledger.\n\n"
  fi
else
...
if [ -n "$META_PROJECT" ]; then
  ...
  if [ "$TRANSFER_STALE" = true ]; then
    # request-reduction v1: ledger-gated like the audit (one in-flight attempt,
    # recorded failures, bounded retries, exhaustion notified once).
    TRANSFER_LEDGER="$META_PROJECT/.living/outputs/knowledge-transfers/.housekeeping-ledger.json"
    [[ "${TRANSFER_TS:-0}" =~ ^[0-9]{1,18}$ ]] || TRANSFER_TS=0
    TRANSFER_DECISION=$(mycelium_housekeeping_decide "knowledge-transfer" "$TRANSFER_LEDGER" "${TRANSFER_TS:-0}" "${HOST_SESSION_ID:-$$}")
    TRANSFER_ACTION=$(printf '%s' "$TRANSFER_DECISION" | mycelium_json_get 'action')
    TRANSFER_ATTEMPT_ID=$(printf '%s' "$TRANSFER_DECISION" | mycelium_json_get 'attempt_id')
    if [ "$TRANSFER_ACTION" = "dispatch" ] && [ -n "$TRANSFER_ATTEMPT_ID" ]; then
      MESSAGES="${MESSAGES}KNOWLEDGE TRANSFER DUE (${TRANSFER_AGE_MSG}; attempt ${TRANSFER_ATTEMPT_ID}): Dispatch ONE background routine subagent (routine model profile; never a second dispatch for this attempt id) to run the mycelium transfer protocol. The subagent should: read recent learnings from all subprojects under ${META_PROJECT}, identify cross-project transfer opportunities, write a report to ${META_PROJECT}/.living/outputs/knowledge-transfers/$(date +%Y-%m-%d).md, write the ISO timestamp to ${TRANSFER_LAST_RUN}, then record completion: python3 ${HERE}/../scripts/housekeeping_ledger.py complete knowledge-transfer --ledger ${TRANSFER_LEDGER} --attempt-id ${TRANSFER_ATTEMPT_ID} (on failure: ... fail knowledge-transfer --ledger ${TRANSFER_LEDGER} --attempt-id ${TRANSFER_ATTEMPT_ID} --reason '<why>'). Do not block on results.\n\n"
    elif [ "$(printf '%s' "$TRANSFER_DECISION" | mycelium_json_get 'notify')" = "True" ]; then
      MESSAGES="${MESSAGES}KNOWLEDGE TRANSFER EXHAUSTED: repeated transfer attempts failed; dispatch is paused for the cooldown. Evidence: ${TRANSFER_LEDGER}. Do not dispatch another transfer worker.\n\n"
    fi
  fi
fi
